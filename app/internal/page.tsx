  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=/internal/home`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
    ...
    <div className="min-h-screen bg-dark-pure flex flex-col items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-dark-card/80 backdrop-blur-sm border border-brand-green/20 rounded-2xl p-8 shadow-card">
          ...
          {error && (
            <div className="mb-6 p-3 bg-accent-coral/10 border border-accent-coral/30 rounded-lg">
